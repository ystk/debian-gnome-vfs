...
/* test-long-cancel.c - Test program for the GNOME Virtual File System.
...
static void
got_file_info (GnomeVFSAsyncHandle *handle,
	       GList *results, /* GnomeVFSGetFileInfoResult* items */
	       gpointer callback_data)
{
	GnomeVFSGetFileInfoResult *res;

	res = results->data;
	
	fprintf (stderr, "Error: got file info (%s), shouldn't happen.\n", gnome_vfs_result_to_string (res->result));
	exit (1);
}
...
main (int argc, char *argv[])
{
	...
	uris = g_list_prepend (NULL, gnome_vfs_uri_new (uri));
	
	gnome_vfs_async_get_file_info (&handle,
				       uris, GNOME_VFS_FILE_INFO_DEFAULT,
